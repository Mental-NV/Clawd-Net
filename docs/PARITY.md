...
| PTY | Interactive command sessions | Legacy relies more on shell/Bash tools and terminal UI than explicit PTY management commands | Keep PTY as a first-class .NET feature and decide how much legacy terminal behavior must map onto it | P1 | Changed | `.NET` PTY is additive and more explicit than the legacy public CLI; transcript persistence added (JSONL-backed, bounded storage, replay support); long-running ergonomics added (timeout, duration tracking, line counting, background session flags, TUI drawer duration/line-count display, timeout warnings) | PTY runtime tests + TUI smoke |
