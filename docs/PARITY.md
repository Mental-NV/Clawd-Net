...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Invocation | Root interactive shell | `claude [prompt]` launches Ink UI by default | `clawdnet` launches TUI by default; keep no-arg interactive launch as primary surface | P0 | Implemented | Current .NET matches the user outcome, but not the root positional prompt shorthand | Manual: `dotnet run --project ClawdNet.App --` |
| Invocation | Root positional prompt | Legacy root accepts `[prompt]` directly | Decide whether to add root prompt shorthand or keep `ask` as the only headless entry | P0 | Changed | Current .NET requires `ask`; this is a migration-visible CLI delta | Smoke: compare `claude -p "hi"` vs `clawdnet ask "hi"` |
| Headless | `-p/--print` text mode | Legacy supports headless one-shot text output from root command | Either add root-compatible headless mode or explicitly keep `ask` as the replacement | P0 | In Progress | `ask` exists, but the root `-p/--print` contract does not | Handler tests + manual smoke |
| Headless | JSON / stream-json output | Legacy supports `text`, `json`, `stream-json`, structured stdin, hook/partial streaming | Current minimum is `ask --json`; decide whether full stream-json parity is required | P0 | In Progress | .NET does not currently expose stream-json, JSON schema, or structured input | Compare stdout shape and exit codes |
| Help | Root help and subcommand help | Legacy Commander help is broad and discoverable | Add explicit `--help` support and per-command help output | P0 | Not Started | Current .NET unknown-command message is short and incomplete | Smoke: `clawdnet --help`, `clawdnet <cmd> --help` |
...
| Session | Session create / list | Legacy has persistent sessions plus interactive resume flows | Preserve create/list and expand to full resume/discovery parity | P0 | In Progress | `.NET` has `session new`, `session list`, and `--session`; no picker / continue / from-pr / rewind | `session list`, interactive `--session`, regression tests |
...
| Model / provider | Provider selection | Legacy is Anthropic-first with extra provider env toggles | Keep provider first-class in .NET; do not regress session-scoped provider behavior | P1 | Changed | `.NET` adds explicit `provider` concept and `provider list/show`; no legacy root equivalent | `provider list`, `ask --provider`, session persistence checks |
...
| Context | System prompt / settings injection | Legacy supports `--settings`, `--system-prompt`, prompt files, append prompt files | Add explicit context injection or document a replacement strategy | P0 | Not Started | Large migration gap; currently no .NET equivalent CLI surface | Fixture-based prompt injection checks |
| Context | `--add-dir`, trust / workspace expansion | Legacy supports explicit extra directories for tool access | Add if migration requires multi-root workspace parity | P1 | Not Started | Current .NET has no visible `--add-dir` equivalent | Manual tool access tests |
...
| Auth | `auth login/status/logout` | Legacy has dedicated auth command group and token helpers | Decide whether .NET will support interactive auth or remain env/config only | P0 | Not Started | Current .NET relies on provider env vars and config; no auth UX | Manual auth story definition needed |
...
| PTY | Interactive command sessions | Legacy relies more on shell/Bash tools and terminal UI than explicit PTY management commands | Keep PTY as a first-class .NET feature and decide how much legacy terminal behavior must map onto it | P1 | Changed | `.NET` PTY is additive and more explicit than the legacy public CLI | PTY runtime tests + TUI smoke |
| Tasks | Task / worker orchestration | Legacy has `tasks` UI and scheduled/background concepts; not the same worker orchestration model | Keep `.NET` task system, but decide what old `/tasks` semantics must be preserved | P1 | Changed | `.NET` worker tasks are additive; legacy scheduled-task compatibility is unresolved | `task list/show/cancel`, TUI task drawer checks |
| Config UI | `/config`, `/theme`, `/color`, `/output-style`, `/model`, `/effort` interactive pickers | Legacy exposes many settings as interactive Ink flows | Move the highest-value ones into TUI drawers/overlays; lower-value flows can fall back to plain text | P1 | Not Started | Major TUI parity area | Manual TUI checks |
| Safety UI | `/permissions`, trust dialog, hook config views, bypass-permissions warnings | Legacy uses Ink modals and menus heavily for safety flows | Preserve as TUI modal/drawer flows, not plain text | P0 | In Progress | Core approval/edit overlays exist, but broader settings and trust surfaces do not | Manual approval and settings checks |
| Reporting | `/status`, `/stats`, `/usage`, `/cost`, `/insights`, `/doctor` | Legacy has rich local or Ink-based reporting flows | Re-add or explicitly replace with simpler CLI/TUI reporting | P1 | Not Started | Reporting/diagnostic parity is still sparse in .NET | Command smoke + fixture-based output checks |
...
| Config compatibility | `~/.claude`, `.claude/settings*.json`, `.mcp.json`, JSONL transcripts | Legacy uses user and project-local layout | Decide whether `.NET` should read legacy layout, migrate it, or intentionally break compatibility | P0 | Changed | Current `.NET` uses app-data JSON stores and separate config files; no compatibility layer is visible | Fixture-based compatibility tests |
...
| Permissions / hooks / privacy settings screens | Browse or edit policy-related settings | Preserve as interactive UI for core safety settings; defer niche settings if necessary | P1 | Medium |
...
| --- | --- | --- | --- |
| Global config root | `CLAUDE_CONFIG_DIR` or `~/.claude` | `<LocalApplicationData>/ClawdNet` or fallback `.clawdnet` next to app | Changed; no visible compatibility layer yet |
| User settings | `~/.claude/settings.json` | No equivalent user settings loader visible | Not compatible yet |
| User memory | `~/.claude/CLAUDE.md` | No equivalent compatibility loader visible | Not compatible yet |
| Project settings | `.claude/settings.json` | No equivalent project settings loader visible | Not compatible yet |
| Project-local overrides | `.claude/settings.local.json` | No equivalent loader visible | Not compatible yet |
| MCP config | project `.mcp.json` plus CLI `--mcp-config` | `config/mcp.json` under app data | Changed; migration path not implemented |
...
| Sessions | JSONL transcripts under `~/.claude/projects/...` | `sessions.json` store | Changed; no legacy transcript import/compat layer |
