...
| `auth` | `status`, `login`, `logout` | OAuth-capable auth with `--browser` login flag, token persistence, and env-var guidance |
...
| Auth | `auth login/status/logout` | Legacy has dedicated auth command group and token helpers | Add OAuth-capable auth flows while preserving current env-var auth and provider inspection | P0 | Implemented | `auth login --browser` initiates OAuth PKCE flow with local callback; `auth status` reports both env-var and OAuth token status with account info; `auth logout` clears OAuth tokens; env-var auth remains fully functional for CI/CD; tokens stored in file-based secure storage (0o600 permissions) with automatic refresh | `auth status`, `auth login`, `auth logout` smoke tests plus AuthCommandHandlerTests and FileTokenStoreTests |
...
| PTY | Interactive command sessions | Legacy relies more on shell/Bash tools and terminal UI than explicit PTY management commands | Keep the current PTY-first `.NET` model and map only a few high-value legacy terminal workflows onto it | P1 | Verified | The current PTY model is accepted and comprehensive: explicit session management via `/pty` commands, TUI drawer with session list and status, fullscreen overlay, transcript persistence (JSONL), timeout support, duration/line tracking, background marking; legacy shell/Bash workflows map onto current PTY sessions; no blocking gaps identified | PTY runtime tests + TUI smoke: `/pty`, `/pty fullscreen`, `/pty close <id>`, F3 PTY drawer |
| Tasks | Task / worker orchestration | Legacy has `tasks` UI and scheduled/background concepts; not the same worker orchestration model | Keep the current worker-task model and add only the minimum scheduled/background semantics that prove necessary | P1 | Verified | The worker-task model is accepted and comprehensive: parent-child delegation (bounded to one level), read-only inspection via `/tasks <id>`, TUI drawer with task list and detail views, task events and status tracking; legacy `/plan` and `/passes` commands deferred to Workflow Command Recovery v1; no blocking gaps for core task inspection and control | `task list/show/cancel`, TUI task drawer checks: `/tasks`, `/tasks <id>`, F4 tasks drawer |
| Config UI | `/config`, `/theme`, `/color`, `/output-style`, `/model`, `/effort` interactive pickers | Legacy exposes many settings as interactive Ink flows | Move only the highest-value settings flows into TUI drawers/overlays; lower-value flows can remain plain text or deferred | P1 | Implemented | `/config` command shows active session configuration (provider, model, session, permission mode) as TUI overlay and REPL text; `/effort` and `/thinking` slash commands implemented for REPL and TUI; `/provider <name> [model]` allows interactive provider/model selection; help overlay (F1) lists all available commands; theme/color/output-style pickers deferred as low migration value | Manual TUI checks: `/config`, `/permissions`, `/effort`, `/thinking`, `/provider`, F1 help overlay |
...
| Workflow UI | `/review`, `/init`, `/init-verifiers`, `/commit`, `/commit-push-pr`, `/branch`, `/diff` | Legacy has workflow-oriented prompt/local commands | Restore most legacy workflow commands that still belong in the first-party CLI | P1 | In Progress | `/commit` and `/review [pr-number]` implemented as slash commands in TUI and REPL; inject workflow prompts that guide model through git/PR workflows using existing `shell` tool; `/branch` already implemented via `session fork` command; `/init` deferred (requires subagent orchestration); `/commit-push-pr`, `/insights` documented as potential future additions; `/diff` deferred (UI-focused) | Manual workflow checks: `/commit`, `/review`, `/review 123` in TUI and REPL |
...
| Secrets / auth assumptions | OAuth + keychain + env in legacy, env/config in `.NET` | Legacy auth model is richer than plain env vars | Add OAuth support while keeping env-var auth available | P0 | Implemented | OAuth browser login is available via `auth login --browser`; tokens persist in file-based secure storage with auto-refresh; env-var auth remains fully functional for CI/CD; macOS keychain integration is deferred | Provider smoke with and without config plus OAuth flow checks |
...
| Help UI | Rich help / shortcut / command browsing | Preserve as interactive overlay or drawer | P1 | Low | Implemented: F1 toggles help overlay showing keyboard shortcuts and slash commands |
| Model / effort / settings pickers | Interactive selection of model, effort, config-related toggles | Preserve as interactive drawer for high-value settings; simplify lower-value toggles if needed | P1 | Medium | Implemented: `/provider <name> [model]` for provider/model selection, `/effort [level]` and `/thinking [mode]` for runtime controls; theme/color/output-style deferred as low migration value |
| Permissions / hooks / privacy settings screens | Browse or edit policy-related settings | Preserve as interactive UI for core safety settings; defer niche settings if necessary | P1 | Medium | Implemented: `/permissions` overlay provides view-only access to current mode and tool category counts; hook config views remain deferred |
...
| Tasks / plan / passes UI | Inspect background work, plan flows, task state | Preserve in TUI drawers and detail views; widen only as orchestration scope firms up | P1 | Medium | Implemented: TUI tasks drawer (F4) shows task list with status, title; `/tasks <id>` shows task detail with events and worker snapshot; `/plan` and `/passes` commands deferred to Workflow Command Recovery v1 as higher-level workflow surfaces |
...
| Install / login / logout / onboarding flows | First-run, auth, companion setup, install helpers | Preserve auth as a first-party flow, but keep install and update outside the CLI for now | P1 | Implemented: OAuth login via `auth login --browser`, env-var guidance preserved, install/update remain outside CLI |
...
| Secrets / auth | env, OAuth, keychain, setup/login flows | provider env vars and OAuth browser login (`auth login --browser`); file-based token storage with auto-refresh | Implemented; OAuth support added while env-var auth remains fully functional; keychain integration deferred |
