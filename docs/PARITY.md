...
| Auth | `auth login/status/logout` | Legacy has dedicated auth command group and token helpers | Decide whether .NET will support interactive auth or remain env/config only | P0 | Implemented | `.NET` now has `auth status` showing provider authentication state; `auth login`/`logout` return helpful messages directing users to env-var-based auth; OAuth/keychain auth from legacy CLI is documented as deferred | `auth status` smoke tests |
