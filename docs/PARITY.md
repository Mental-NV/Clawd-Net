...
| Plugin manager / marketplace UI | Discover, install, update, enable, disable plugins | Local lifecycle implemented as CLI commands (install/uninstall/enable/disable/status/validate/reload with `--all` and `--keep-data`); marketplace UI deferred until plugin distribution model is settled | P1 | High: current `.NET` has full local lifecycle support; marketplace and update UI remain deferred |
